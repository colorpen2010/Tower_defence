...
class Wave():
    ...
    def enemy_creating(self,type, hp, ret=False):
        ...
        if type == 'blue':
            ...
            enemy = enemy_factory.enem_factory(type, self.tec_level.give_your_number_of_etashey(), True, [1, 1, 0.5, 0.5],
                                               scorost=1.50,
    ...
    def controller(self,events):
        for o in events:
            if self.count <= 0 and self.types != []:
                del self.types[0]
                #выбор следующего противника
                if self.types != []:
                    first_enemy = self.types[0]
                    self.enemy_type = list(self.first_enemy.keys())[0]
                    self.enemy_settings = self.first_enemy[self.enemy_type]
                    count = self.enemy_settings['count']
